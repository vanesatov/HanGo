...
import com.example.hango.databinding.ActivityColoresFinalBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase

class ColoresFinalActivity : AppCompatActivity() {
    private lateinit var binding: ActivityColoresFinalBinding
    private lateinit var prefs: SharedPreferences

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityColoresFinalBinding.inflate(layoutInflater)
        ...
        if (userId != null) {
            ...
            userRef.child("nivel").get().addOnSuccessListener { dataSnapshot ->
                ...
                if (nivelFirebase < 9) {
                    userRef.child("nivel").setValue(9)
                    prefs.edit().putInt("nivel", 9).apply()
                    binding.txtSubidaNivel.visibility = View.VISIBLE
                } else {
                    binding.txtSubidaNivel.visibility = View.GONE
                }
                userRef.child("lecciones").child("colores").child("completada").setValue(true)
            }
        }
    }
