...
import com.example.hango.databinding.ActivityFrutas1Binding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch

class Frutas1Activity : AppCompatActivity() {
    private lateinit var binding: ActivityFrutas1Binding
    private var mediaPlayer: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityFrutas1Binding.inflate(layoutInflater)
        ...
        binding.btnSiguiente.setOnClickListener {
            val intent = Intent(this, Frutas2Activity::class.java)
    ...
    private fun reproducirSonido() {
        mediaPlayer?.release()
        mediaPlayer = MediaPlayer.create(this, R.raw.manzana)
        mediaPlayer?.start()
    ...
    private fun marcarLeccionComoAbiertaSiEsPrimeraVez() {
        ...
        val dbRef = FirebaseDatabase.getInstance().reference
            .child("usuarios").child(uid).child("lecciones").child("frutas")
