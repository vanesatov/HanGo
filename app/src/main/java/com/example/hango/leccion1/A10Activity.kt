...
import com.example.hango.databinding.ActivityA10Binding
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch

class A10Activity : BaseLeccionActivity() {
        private lateinit var binding: ActivityA10Binding
        private var mediaPlayer: MediaPlayer? = null

        override fun onCreate(savedInstanceState: Bundle?) {
            super.onCreate(savedInstanceState)
            enableEdgeToEdge()
            binding = ActivityA10Binding.inflate(layoutInflater)
        ...
        private fun reproducirSonido() {
            mediaPlayer?.release()
            mediaPlayer = MediaPlayer.create(this, R.raw.letra_eu)
            mediaPlayer?.start()
