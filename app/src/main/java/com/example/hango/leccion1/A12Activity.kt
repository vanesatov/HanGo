...
import com.example.hango.databinding.ActivityA12Binding
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch

class A12Activity : BaseLeccionActivity() {
    private lateinit var binding: ActivityA12Binding
    private var mediaPlayer: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityA12Binding.inflate(layoutInflater)
    ...
    private fun reproducirSonido() {
        mediaPlayer?.release()
        mediaPlayer = MediaPlayer.create(this, R.raw.letra_ya)
        mediaPlayer?.start()
