...
import com.example.hango.databinding.ActivityA16Binding
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch

class A16Activity : BaseLeccionActivity() {
    private lateinit var binding: ActivityA16Binding
    private var mediaPlayer: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityA16Binding.inflate(layoutInflater)
    ...
    private fun reproducirSonido() {
        mediaPlayer?.release()
        mediaPlayer = MediaPlayer.create(this, R.raw.letra_yu)
        mediaPlayer?.start()
