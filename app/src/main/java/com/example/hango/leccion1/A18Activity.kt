...
import com.example.hango.databinding.ActivityA18Binding
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch

class A18Activity : BaseLeccionActivity() {
    private lateinit var binding: ActivityA18Binding
    private var mediaPlayer: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityA18Binding.inflate(layoutInflater)
    ...
    private fun reproducirSonido() {
        mediaPlayer?.release()
        mediaPlayer = MediaPlayer.create(this, R.raw.letra_yo)
        mediaPlayer?.start()
