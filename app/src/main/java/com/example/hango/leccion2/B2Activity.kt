...
import com.example.hango.databinding.ActivityB2Binding
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch

class B2Activity : BaseLeccionActivity() {
    private lateinit var binding: ActivityB2Binding
    private var mediaPlayer: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityB2Binding.inflate(layoutInflater)
    ...
    private fun reproducirSonido() {
        mediaPlayer?.release()
        mediaPlayer = MediaPlayer.create(this, R.raw.letra_ae)
        mediaPlayer?.start()
