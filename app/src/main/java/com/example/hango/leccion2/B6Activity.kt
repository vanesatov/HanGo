...
import com.example.hango.databinding.ActivityB6Binding
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch

class B6Activity : BaseLeccionActivity() {
    private lateinit var binding: ActivityB6Binding
    private var mediaPlayer: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityB6Binding.inflate(layoutInflater)
    ...
    private fun reproducirSonido() {
        mediaPlayer?.release()
        mediaPlayer = MediaPlayer.create(this, R.raw.letra_yae)
        mediaPlayer?.start()
