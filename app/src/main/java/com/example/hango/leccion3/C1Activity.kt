...
import com.example.hango.databinding.ActivityC1Binding
...
class C1Activity : AppCompatActivity() {
    private lateinit var binding: ActivityC1Binding
    private var mediaPlayer: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityC1Binding.inflate(layoutInflater)
    ...
    private fun reproducirSonido() {
        mediaPlayer?.release()
        mediaPlayer = MediaPlayer.create(this, R.raw.letra_ga)
        mediaPlayer?.start()
    ...
    private fun marcarLeccionComoAbiertaSiEsPrimeraVez() {
        ...
        val dbRef = FirebaseDatabase.getInstance().reference
            .child("usuarios").child(uid).child("lecciones").child("leccion3")
