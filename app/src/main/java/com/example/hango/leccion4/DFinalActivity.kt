...
import com.example.hango.databinding.ActivityDfinalBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase

class DFinalActivity : AppCompatActivity() {
    private lateinit var binding: ActivityDfinalBinding
    private lateinit var prefs: SharedPreferences

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityDfinalBinding.inflate(layoutInflater)
        ...
        if (userId != null) {
            ...
            userRef.child("nivel").get().addOnSuccessListener { dataSnapshot ->
                ...
                if (nivelFirebase < 4) {
                    userRef.child("nivel").setValue(4)
                    prefs.edit().putInt("nivel", 4).apply()
                    binding.txtSubidaNivel.visibility = View.VISIBLE
                } else {
                    binding.txtSubidaNivel.visibility = View.GONE
                }
                userRef.child("lecciones").child("leccion4").child("completada").setValue(true)
            }
        }
    }
