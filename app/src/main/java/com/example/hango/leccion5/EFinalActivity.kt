package com.example.hango.leccion5
...
import com.example.hango.databinding.ActivityEfinalBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase

class EFinalActivity : AppCompatActivity() {
    private lateinit var binding: ActivityEfinalBinding
    private lateinit var prefs: SharedPreferences

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityEfinalBinding.inflate(layoutInflater)
        ...
        if (userId != null) {
            ...
            userRef.child("nivel").get().addOnSuccessListener { dataSnapshot ->
                ...
                if (nivelFirebase < 5) {
                    userRef.child("nivel").setValue(5)
                    prefs.edit().putInt("nivel", 5).apply()
                    binding.txtSubidaNivel.visibility = View.VISIBLE
                } else {
                    binding.txtSubidaNivel.visibility = View.GONE
                }
                userRef.child("lecciones").child("leccion5").child("completada").setValue(true)
            }
        }
    }
