...
import com.example.hango.databinding.ActivityFfinalBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase

class FFinalActivity : AppCompatActivity() {
    private lateinit var binding: ActivityFfinalBinding
    private lateinit var prefs: SharedPreferences

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityFfinalBinding.inflate(layoutInflater)
        ...
        if (userId != null) {
            ...
            userRef.child("nivel").get().addOnSuccessListener { dataSnapshot ->
                ...
                if (nivelFirebase < 6) {
                    userRef.child("nivel").setValue(6)
                    prefs.edit().putInt("nivel", 6).apply()
                    binding.txtSubidaNivel.visibility = View.VISIBLE
                } else {
                    binding.txtSubidaNivel.visibility = View.GONE
                }
                userRef.child("lecciones").child("leccion6").child("completada").setValue(true)
            }
        }
    }
