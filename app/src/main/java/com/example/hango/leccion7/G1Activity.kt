...
import com.example.hango.databinding.ActivityG1Binding
...
class G1Activity : AppCompatActivity() {
    private lateinit var binding: ActivityG1Binding
    private var mediaPlayer: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityG1Binding.inflate(layoutInflater)
        ...
        binding.btnSiguiente.setOnClickListener {
            val intent = Intent(this, G2Activity::class.java)
    ...
    private fun reproducirSonido() {
        mediaPlayer?.release()
        mediaPlayer = MediaPlayer.create(this, R.raw.letra_ji)
        mediaPlayer?.start()
    ...
    private fun marcarLeccionComoAbiertaSiEsPrimeraVez() {
        ...
        val dbRef = FirebaseDatabase.getInstance().reference
            .child("usuarios").child(uid).child("lecciones").child("leccion7")
