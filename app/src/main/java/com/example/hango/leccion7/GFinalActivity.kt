...
import com.example.hango.databinding.ActivityGfinalBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase

class GFinalActivity : AppCompatActivity() {
    private lateinit var binding: ActivityGfinalBinding
    private lateinit var prefs: SharedPreferences

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityGfinalBinding.inflate(layoutInflater)
        ...
        if (userId != null) {
            ...
            userRef.child("nivel").get().addOnSuccessListener { dataSnapshot ->
                ...
                if (nivelFirebase < 7) {
                    userRef.child("nivel").setValue(7)
                    prefs.edit().putInt("nivel", 7).apply()
                    binding.txtSubidaNivel.visibility = View.VISIBLE
                } else {
                    binding.txtSubidaNivel.visibility = View.GONE
                }
                userRef.child("lecciones").child("leccion7").child("completada").setValue(true)
            }
        }
    }
